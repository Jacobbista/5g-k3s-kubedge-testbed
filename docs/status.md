...
| Standalone `synthetic-adapter` (phase 10, positioning_engine) | Supported * | manual | off | Tier-1 synthetic source in the lean baseline; the engine discovers it via its `devices` capability and it exercises the real `/measurement` HTTP contract end to end |
...
| Custom adapter catalog (`wifi-adapter`, `vendor-adapter`, bring-your-own image) | Experimental | — | off | deployed on demand from the Northbound dashboard console, not by Ansible |
...
| Edge apps platform (phase 12: local registry + deploy-from-image + dynamic route) | Experimental | — | off | opt-in via `testbed apps on`; in-cluster `registry:2` (insecure HTTP + basic-auth, NodePort only) + `apps` namespace + dashboard Apps page (deploy/list/delete, `allow_workload_create` gated); front-door routes `<name>.<base>` dynamically; not yet exercised e2e. See [architecture/edge-apps.md](architecture/edge-apps.md) |
