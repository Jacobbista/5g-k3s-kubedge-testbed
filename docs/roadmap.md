...
- **libvirt/KVM as an alternative provider.** VirtualBox is the default provider because it runs the same way on Linux, macOS, and Windows, which is what a testbed meant to be reproduced on an arbitrary laptop needs. On a Linux host a `vagrant-libvirt` provider, selected per host alongside VirtualBox rather than replacing it, is the more native path: KVM is in-tree, so the hypervisor moves with the kernel instead of an out-of-tree module rebuilt against each one; the network fast path drops into the kernel through vhost-net and virtio; and VFIO/PCI passthrough becomes available, the one capability VirtualBox does not offer and the reason to reach for KVM if the UPF ever drives a real NIC or DPDK. The Ansible automation is provider-agnostic, so the change is confined to the `Vagrantfile` and the `testbed-config` host checks: a libvirt-format box in place of `ubuntu/jammy64`, synced folders over virtiofs, and the bridged physical-RAN interface expressed with `trustGuestRxFilters` instead of `--nicpromisc3`. Additive by design, with the VirtualBox default kept as the portable path.
...
- **Server-side session store for the front-door gate (Redis).** The `frontdoor_gate` oauth2-proxy stores the whole session, access and refresh tokens included, inside its own cookie, which is why it splits into `_oauth2_proxy_0`/`_1` past 4KB. Moving the store to Redis leaves only a session identifier in the browser and buys two things a shared testbed eventually needs: tokens stop living on the client, and a session becomes genuinely revocable (delete the key and that browser is out immediately, instead of a stolen cookie staying valid until it expires). The cost is one more component to keep running, and every session drops if it restarts without persistence. Worth doing when the gate fronts more than one service and the user set is no longer a handful of known people, which is the direction implied by running this as a private 5G cluster shared with research groups. See [security/iam.md](security/iam.md).
